  ...
  <div class="app-container">
    ...
    >
        ...
        <template slot-scope="{row}">
          <div class="media-content">
            ...
            <div>
              ...
              <div>拼团价格：￥ {{row.price}}</div>
            </div>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="成团人数" width="100px" align="center">
        <template slot-scope="{row}">
          <span>{{row.p_num}}</span>
        </template>
      </el-table-column>

      <el-table-column label="拼团时限（小时）" width="80px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="拼团状态" width="80px" align="center">
        <template slot-scope="{row}">
          <span :style="{color: row.time_status === '拼团中' ? 'red':'#888'}">
            {{ row.time_status }}
          </span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" >
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="拼团人数">
          <el-input-number v-model="temp.p_num"  :min="0"></el-input-number>
        </el-form-item>
